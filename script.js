/* =====================================================================
   VISUALIZADOR DIDÁTICO DE ÁRVORE B+ (ORDEM 4)
   =====================================================================

   Este arquivo implementa, em JavaScript bem simples, uma Árvore B+
   de ordem 4. As regras da ordem 4 são:

     - Cada nó pode ter, no máximo, 3 CHAVES.
     - Cada nó interno pode ter, no máximo, 4 FILHOS.
     - Todo nó (exceto a raiz) precisa ter, no mínimo, 1 CHAVE.
       (fórmula geral: mínimo = ceil(ordem / 2) - 1 = ceil(4/2) - 1 = 1)

   Características de uma Árvore B+ (diferente de uma árvore B comum):

     - Todos os DADOS ficam guardados nas FOLHAS.
     - Os NÓS INTERNOS não guardam dados de verdade: eles só têm chaves
       "separadoras", usadas apenas para decidir por qual caminho descer
       durante uma busca.
     - As FOLHAS são ligadas entre si (campo "proximo"), formando uma
       lista do menor para o maior valor, o que facilita percorrer
       todos os dados em ordem.

   Estrutura de um nó (um simples objeto JavaScript):

     {
       folha: true/false,   -> true = nó folha (guarda dados)
                                false = nó interno (só separadores)
       chaves: [ ... ],     -> lista de números guardados no nó, em ordem
       filhos: [ ... ],     -> lista de nós filhos (só usada em nós internos)
       proximo: outroNo     -> aponta para a próxima folha (só usado em folhas)
     }
